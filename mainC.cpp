...
void doiMK() {
    char nhapTK[50], nhapMK[50], MKmoi[50];
    char TK[50], MK[50], VT[50];
    int found = 0;
    FILE *infile, *outfile;
...
nhaplai:
    printf("Vui long nhap tai khoan: ");
    fgets(nhapTK, sizeof(nhapTK), stdin);
    nhapTK[strcspn(nhapTK, "\n")] = '\0'; 
    ...
    while (fscanf(infile, "%s %s %s", TK, MK, VT) != EOF) {
        if (strcmp(nhapTK, TK) == 0 && strcmp(nhapMK, MK) == 0) {
            printf("Vui long nhap mat khau moi: ");
            fgets(MKmoi, sizeof(MKmoi), stdin);
            MKmoi[strcspn(MKmoi, "\n")] = '\0'; 
    ...
    if (found) {
        remove("user.txt");
        rename("temp.txt", "user.txt");
        printf("____DOI MAT KHAU THANH CONG____\n");
    } else {
        printf("____TAI KHOAN MAT KHAU KHONG CHINH XAC____\n");
        printf("____VUI LONG NHAP LAI____\n");
        goto nhaplai;
    }
}
